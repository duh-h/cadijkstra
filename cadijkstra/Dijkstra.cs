...
{
    ShortestPathResult FindShortestPath(Node from, Node to);
}
...
public class Dijkstra : IShortestPathFinder
{
    ...
    class VisitingData
    {
        ...
        public bool HasScheduledVisits
        {
            get { return _scheduled.Count > 0; }
        }
    ...
    public ShortestPathResult FindShortestPath(Node from, Node to)
    {
        ...
        var computedPath = control.ComputedPathToOrigin(to);
        var pathList = new List<Node>();

        foreach (var node in computedPath)
        {
            pathList.Insert(0, node);
        }

        int distance = control.QueryWeight(to).Value;

        return new ShortestPathResult
        {
            Path = control.HasComputedPathToOrigin(to) ? pathList.ToArray() : null,
            Distance = distance
        };
    }
}